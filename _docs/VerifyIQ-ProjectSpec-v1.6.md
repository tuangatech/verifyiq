...
Use cases fall into two categories by entry point. **UC-1 through UC-4** are user-initiated: a person submits a verification request through the Next.js UI, which calls `POST /verify` on the Orchestrator. Each exercises a different combination of agents and data paths, making the core A2A patterns — fan-out, optional agent handling, graceful degradation, sequential chaining — visible in isolation. **UC-5** is platform-initiated: an external Mortgage Platform Orchestrator calls VerifyIQ as a registered A2A sub-agent via `POST /tasks/send`. The internal pipeline is identical in both categories; what differs is the entry point and who receives the result.
...
The user fills a form with: subject name, simulated subject ID, use case type (mortgage / rental / auto / hire), whether the subject has a foreign address on record, and a consent checkbox. The Orchestrator receives the request, assigns a `correlation_id` that flows through the entire pipeline, returns a `task_id` immediately, and begins processing in the background.
...
SSE (Server-Sent Events) is a one-directional HTTP streaming mechanism where the server pushes events to the client over a persistent connection without the client polling. The Orchestrator emits progress events at each milestone — "Agents resolved", "Credit report received", "Employment verified", "Running risk synthesis" — and every event carries the `correlation_id` for tracing. SSE streams to two different consumer types depending on the entry point: the **browser** (UC-1 to UC-4, via the Next.js report view) receives a human-readable live timeline; the **Mortgage Platform Orchestrator** (UC-5) receives the same event stream as structured JSON it can log or forward. The mechanism is identical — ADK on the server side manages the streaming connection regardless of which consumer is listening.

### F-6: Structured Verification Report
The web UI displays collapsible sections: credit summary, employment summary, international summary (or failure/timeout notice), and the risk decision with decision factors and reasoning.

### F-7: Agent Network Dashboard
Shows all agents registered with the Registry — name, URL, skills, health, average latency, last seen. Updated every 10 seconds.

### F-8: Request History
Past verification requests with subject name, use case, timestamp, decision, and link to full report.
...
|---|---|---|
| **Frontend** | Next.js 14 (App Router) | Web UI — verification form, live report view, agent dashboard |
| **Frontend styling** | Tailwind CSS + shadcn/ui | Component library and utility-first styling |
| **Frontend streaming** | Native `EventSource` API | Consumes SSE stream from Orchestrator |
...
User (browser)
 └→ Next.js UI → POST /verify → Orchestrator (:8000)
                                    ├→ [queries Registry]
...
### 6.1 Frontend — Next.js Web UI (:3000)

**Pages:**
- `/` — Verification form: subject name, simulated ID, use case dropdown, foreign address checkbox, consent toggle
- `/reports/[task_id]` — Live report: SSE progress timeline + collapsible artifact sections
- `/agents` — Agent Network: Registry contents, health, latency, last seen
- `/history` — Past requests table with links to reports

**Key interactions:**
- `POST /verify` → `task_id` returned immediately
- `GET /verify/{task_id}/stream` → SSE connection; each event carries `correlation_id`
- `/agents` polls `GET /orchestrator/agents` every 10 seconds
...
- **Caller role (UC-1 to UC-4):** Receives a request from the user via `POST /verify`, resolves agents from the Registry, fans out tasks to data agents, chains to Risk Synthesis, and streams progress back to the browser via SSE.
...
| `GET /verify/{task_id}/stream` | GET | SSE stream for browser clients |
| `GET /verify/{task_id}` | GET | Poll for task status (browser or external caller) |
...
| `GET /agents` | GET | Proxies Registry contents for the frontend dashboard |
...
`SSEStreamer` — Publishes progress events to connected clients (browser or external orchestrator) with `correlation_id` on every event.
...
**`sse_events`** — One row per SSE event emitted to the frontend. Stores event type (progress / artifact / completed / error), full JSON payload, and `correlation_id`. Enables replaying the exact sequence of events the UI received for any historical request.
...
- UC-1a: all four agents invoked, international data in report, decision returned
- UC-1b: international `unavailable`, report notes it, decision still returned
- UC-2: only Equifax + Employment + Synthesis invoked; International task row has status `skipped`
- UC-3: only Equifax + Employment + Synthesis invoked; Risk Synthesis prompt weighted toward debt-to-income ratio
- UC-4: only International + Employment + Synthesis invoked; Equifax task row has status `skipped`
- All `correlation_id` values consistent across all SQLite rows for each request
- UC-5 (Mortgage Platform inbound A2A) tests belong in Phase 9 — see Build Sequence.
...
**SQLite audit log (`agent_tasks`):** Full input/output/error/`started_at`/`ended_at`/`correlation_id` per task. Queryable from Agent Network page or `sqlite3` CLI.
...
**SSE event log (`sse_events`):** Every event emitted to the frontend persisted with `correlation_id`. Enables replay of the exact UI event sequence for any historical request.

**Agent Network dashboard:** Registry state every 10 seconds. Health, latency, last seen.
...
║  │                                                            │  ║
║  │  ┌──────────────┐   ┌──────────────────────────────────┐  │  ║
║  │  │  ui          │   │  Docker internal network         │  │  ║
║  │  │  :3000       │   │  (verifyiq_default)              │  │  ║
║  │  └──────┬───────┘   │                                  │  │  ║
║  │         │ HTTP/SSE  │  ┌────────────┐  :8000           │  │  ║
║  │         └──────────►│  │orchestrator│◄─────────────┐   │  │  ║
║  │                     │  └─────┬──────┘              │   │  │  ║
...
| Service | Port | Notes |
|---|---|---|
| Next.js UI | 3000 | User-facing frontend |
| Orchestrator | 8000 | Hub — serves both browser and external A2A callers |
| Agent Registry | 8099 | |
| Equifax Agent | 8001 | |
...
`verifyiq.db` + `registry.db` → Neon PostgreSQL, Docker Compose → Railway, Next.js → Vercel. Only connection strings change — no code changes.
...
Each agent: `python:3.11-slim` base, copies `agents/shared/` and the agent directory, installs `requirements.txt`, exposes its port. UI: `node:20-alpine`.
...
### Phase 7 — Frontend (4–6 hours)

Build the Next.js UI. Add the `ui` service to `docker-compose.yml`.

- `/` — Verification form: subject name, simulated ID, use case dropdown, foreign address checkbox, consent toggle
- `/reports/[task_id]` — SSE-connected live report: progress timeline, collapsible artifact sections, raw JSON toggle
- `/agents` — Agent Network dashboard from `GET /orchestrator/agents`, polling every 10 seconds
- `/history` — Past requests table with subject name, use case, timestamp, decision, and report link

**Tests:** All End-to-End Flow tests from Section 10 (UC-1a, UC-1b, UC-2, UC-3, UC-4). Manual smoke test of each.

**Goal:** Full usable product. No Swagger or `curl` required to run a verification end-to-end.
...
Build the `mortgage-platform` service — a minimal ADK agent (~60 lines) at `:9000`. On startup it discovers VerifyIQ's Agent Card, confirms the `verify_subject` skill, dispatches an A2A task with a sample mortgage subject, connects to the SSE stream, and logs progress events and the final `VerificationDecision` to stdout. Added to `docker-compose.yml` with `depends_on: orchestrator`; started explicitly (`docker compose up mortgage-platform`) so it does not interrupt the default UC-1–7 workflow.
...
| SSE streaming to caller | `SSEStreamer` pushing to browser — already specced | **ADK manages backpressure, reconnects to external caller** |
...
*Document version: 1.5 | Last updated: 2026-04-08 | Status: Ready for implementation*
