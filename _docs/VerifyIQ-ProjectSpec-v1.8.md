...
### Project Specification v1.8
*A side project for learning Agent-to-Agent (A2A) multi-agent orchestration patterns*

> **v1.8 changes:** ADK (Google Agent Development Kit) removed from the project entirely.
> The Orchestrator's inbound A2A callee endpoints (Phase 9, UC-5) are implemented in plain
> FastAPI — the same stack used everywhere else. The Mortgage Platform service is also plain
> FastAPI + httpx. Rationale: ADK is a full agent framework tied to Google's ecosystem, not
> a lightweight A2A protocol library. It hides the protocol mechanics this project exists to
> learn, pulls in heavy dependencies (Gemini SDK, Google Auth) for no benefit in a
> model-agnostic project, and doesn't solve the hard problems (Registry discovery,
> fail-partial orchestration, outcome bundles). Hand-rolling the ~80 lines of A2A server
> endpoints teaches more and keeps the stack uniform. Section 16 subsection on ADK replaced
> with "Why Not ADK?" explaining the decision. All ADK references removed from Sections 1–6,
> 14, and the tech stack table. Section 16 title updated.
>
...
16. [Why A2A — Not Hardwired Workflows, MCP Tools, LangGraph, or ADK](#16-why-a2a--not-hardwired-workflows-mcp-tools-langgraph-or-adk)
...
- **Hand-rolled A2A server** — implementing inbound task handling for external orchestrators you don't control, including task lifecycle management, SSE streaming back to the caller, and Agent Card advertisement, in plain FastAPI
...
**Who:** An external Mortgage Platform Orchestrator — a separate service built by a different team — that manages the full loan origination pipeline (document collection, appraisal scheduling, underwriting, rate locking). It needs subject verification and calls VerifyIQ as a registered sub-agent.

**Setup:** The Mortgage Platform Orchestrator is a second Docker Compose service (`mortgage-platform`) running a plain FastAPI + httpx service. It discovers VerifyIQ via VerifyIQ's Agent Card at `http://orchestrator:8000/.well-known/agent.json`. It then dispatches an A2A task to `POST http://orchestrator:8000/tasks/send` with a verification request payload.

**What VerifyIQ's Orchestrator does:** Receives the inbound A2A task via plain FastAPI endpoints. The Orchestrator manages the task lifecycle back to the Mortgage Platform — accepting the task, returning a `task_id` immediately, streaming progress events via SSE, and delivering the final `VerificationDecision` artifact as the task result. VerifyIQ internally runs its standard pipeline (Equifax → Employment → Intl → Risk Synthesis) exactly as it does for UC-1a, then sends the result back to the caller via the A2A protocol.
...
```
Mortgage Platform Orchestrator (FastAPI + httpx)    ← external team, separate codebase
    └─ A2A → VerifyIQ Orchestrator (FastAPI)
                  ├─ A2A → Equifax Agent
...
**Key A2A learning:** This is the scenario where the Orchestrator becomes a true A2A peer — not just a caller, but a callee. The Orchestrator's inbound A2A endpoints (`POST /tasks/send`, `GET /tasks/{task_id}`, `GET /tasks/{task_id}/stream`) are implemented in plain FastAPI, the same stack used for all other endpoints. This is ~80 lines of code that manages: task receipt and validation, immediate `task_id` acknowledgement, task lifecycle state machine visible to the caller, SSE streaming back to the external consumer, and final artifact delivery. Implementing this by hand (rather than importing a framework) makes the A2A server protocol fully visible and understood.
...
| **Orchestrator** | Python + FastAPI | Hub agent — discovery, fan-out, chaining, SSE emission, inbound A2A callee |
...
**Plain FastAPI for inbound A2A handling (no ADK).** The Orchestrator's callee endpoints (`POST /tasks/send`, `GET /tasks/{id}`, `GET /tasks/{id}/stream`) are implemented in plain FastAPI — the same stack used for all other endpoints. ADK was considered and rejected: it is a full agent framework tied to Google's ecosystem (Gemini, Vertex AI), not a lightweight A2A library. It would pull heavy dependencies for no benefit in a model-agnostic project, and it hides the protocol mechanics this project exists to learn. The inbound A2A server code is ~80 lines of FastAPI — straightforward, fully understood, and consistent with the rest of the codebase. See Section 16 for the full rationale.
...
```
┌──────────────────────────────────────────────────────────────────┐
│      MORTGAGE PLATFORM ORCHESTRATOR  (FastAPI + httpx)  :9000    │
...
│                                                                  │
│  Inbound A2A: receives tasks, manages lifecycle back to          │
│  Mortgage Platform, streams SSE to external caller               │
│                                                                  │
│  ┌──────────────────────┐  ┌──────────────┐  ┌───────────────┐  │
...
```mermaid
graph TD
    MP["Mortgage Platform Orchestrator<br/>(FastAPI + httpx) :9000<br/><i>external team — separate codebase</i>"]
    ORCH["Orchestrator Agent<br/>(FastAPI) :8000"]
...
Mortgage Platform Orchestrator (:9000)    ← external team
 └→ A2A: POST /tasks/send → Orchestrator (:8000)   [FastAPI receives]
                                    ├→ [queries Registry]
                                    ├→ [parallel] → Equifax Agent (:8001)
                                    ├→ [parallel] → Employment Agent (:8002)
                                    ├→ [parallel, optional] → Intl Agent (:8003)
...
In UC-5, the Orchestrator plays **both roles simultaneously**: it is a callee (A2A server, receiving the Mortgage Platform's task via `POST /tasks/send`) and a caller (A2A client, dispatching outbound tasks to data agents). The data agents below it see no difference — they receive the same A2A tasks regardless of who initiated the top-level request.
...
### 6.3 Orchestrator Agent — FastAPI (:8000)
...
- **Callee role (UC-5):** Receives an inbound A2A task from the Mortgage Platform Orchestrator via `POST /tasks/send`. Plain FastAPI endpoints handle the server-side protocol — task receipt, state machine management, SSE streaming back to the external caller, and final artifact delivery. Internally the same pipeline runs as in UC-1 to UC-4.

**Both paths use the same stack.** The callee endpoints are ~80 lines of FastAPI — `POST /tasks/send` validates input and starts the pipeline in the background, `GET /tasks/{id}` exposes task state, `GET /tasks/{id}/stream` forwards internal SSE events to the external caller. The outbound dispatch to data agents uses plain `httpx` wrapped in `TaskDispatcher`.
...
## 16. Why A2A — Not Hardwired Workflows, MCP Tools, LangGraph, or ADK

This section is the complete architectural rationale for every technology choice at the inter-agent boundary. It covers four alternatives that were ruled out (hardwired workflows, MCP tools, LangGraph, ADK) and explains why plain FastAPI + httpx is the right stack for implementing A2A in a learning project.
...
ADK (Google Agent Development Kit) is Google's Python framework for building A2A-compliant agents. It provides Agent Card generation, an A2A client for dispatching tasks, a task state machine, and server-side task handling abstractions. It was considered for the Orchestrator's inbound A2A endpoints (UC-5) and rejected for three reasons.

**1. ADK is a full agent framework, not an A2A library.** ADK wants to own your agent runtime — it brings its own agent base classes, session management, tool system, and deep integration with Google's ecosystem (Gemini, Vertex AI). VerifyIQ is deliberately model-agnostic (OpenRouter with swappable model strings). Importing ADK to get A2A server endpoints would pull in Gemini SDK, Google Auth, and other dependencies that provide zero value here while adding version conflicts and container bloat.

**2. ADK hides the protocol mechanics this project exists to learn.** The inbound A2A server code — accepting a task, managing state transitions visible to the caller, streaming SSE events, delivering the final artifact — is ~80 lines of FastAPI. That code makes the A2A server protocol fully visible and understood. Delegating it to ADK would replace learning with importing. After building `TaskDispatcher`, `SSEStreamer`, and `TaskManager` by hand for the outbound path, the inbound path uses the same patterns in reverse. There is nothing conceptually new that ADK would teach.

**3. ADK doesn't solve the hard problems.** VerifyIQ's interesting complexity — Registry-based dynamic discovery, latency-aware agent selection, fail-partial orchestration, structured outcome bundles with failure context, use-case-specific DAG routing — is all custom code. ADK doesn't provide any of it. ADK only replaces the HTTP plumbing, which is the easiest part of the system.

**When ADK would be the right choice:** If you are building agents from scratch in Google's ecosystem (Gemini + Vertex AI), need production-grade task lifecycle management for high-concurrency callers, or don't need to understand the A2A protocol internals. ADK is a legitimate framework for production agent systems — it's just the wrong tool for a project whose goal is understanding the protocol.

**Where ADK does NOT appear in the codebase:** Nowhere. All services — Orchestrator, data agents, Mortgage Platform — use plain FastAPI + httpx. The A2A protocol is implemented directly, not abstracted by a framework.
...
| Dimension | A2A (inter-agent) | Hardwired workflow | MCP Tools | LangGraph | ADK |
|---|---|---|---|---|---|
| **Scope** | Across org/deployment boundaries | Same codebase, any boundary | Within one agent, same team | Within one agent, stateful branching | Full agent framework (Google ecosystem) |
| **Lives in** | Separate process + container | Same process or tightly coupled | Same process | Same process | Same process; owns the runtime |
| **Called via** | HTTP over network | Direct function call / import | Local function call | Graph node execution | Framework abstractions |
| **Team ownership** | Each agent independently owned | One team owns everything | Same team owns all tools | Same team owns all nodes | Same team; Google-ecosystem-aligned |
| **Deploy independently** | ✅ Yes | ❌ No | ❌ No | ❌ No | ✅ Yes (agents are independent) |
| **Language agnostic** | ✅ Any language | ❌ Must share language | ❌ Must match agent language | ❌ Python only | ❌ Python only |
| **Where in VerifyIQ** | Orchestrator → all agents | ❌ Not used | Inside each agent (plain Python) | Inside Employment Agent | ❌ Not used — protocol learned by hand |
| **Best for** | Fixed pipelines across org boundaries | Single-team owned automation | Internal agent operations | Internal branching state machines | Production agent systems in Google ecosystem |
