...
- **ADK server-side only** -- ADK handles inbound A2A tasks from external callers (UC-5: Mortgage Platform) where managing task lifecycle, SSE backpressure, and cancellation for a caller you don't control is non-trivial. Outbound dispatch to data agents uses plain `httpx` -- simple enough to own directly.
