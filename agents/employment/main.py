...
from agents.employment.tools import (
    build_employment_seed,
    call_llm_employment_profile,
    validate_employment_artifact,
...
async def tasks_send(task: A2ATask) -> A2ATaskResult:
    """Accept an A2A task, call LLM to generate employment verification, validate, and return."""
    started_at = datetime.now(timezone.utc).isoformat()
    seed = build_employment_seed(task.input)

    raw = await call_llm_employment_profile(seed)
    try:
        artifact = validate_employment_artifact(raw)
